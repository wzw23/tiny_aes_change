...
static void test_encrypt_ecb_verbose(void)
{
    ...
    uint8_t plain_text[64] = { 
                               ...
                               (uint8_t) 0x6b, (uint8_t) 0xc1, (uint8_t) 0xbe, (uint8_t) 0xe2, (uint8_t) 0x2e, (uint8_t) 0x40, (uint8_t) 0x9f, (uint8_t) 0x96, (uint8_t) 0xe9, (uint8_t) 0x3d, (uint8_t) 0x7e, (uint8_t) 0x11, (uint8_t) 0x73, (uint8_t) 0x93, (uint8_t) 0x17, (uint8_t) 0x2a,
                               };
    ...
    printf("AES_ll automatic verification\n");
    for (i = 0; i < 4; ++i)
    { static int count =0;
      count++;
      printf("Test:%d\n",count);
      printf("operate:encipher\n");
      printf("key:");
      phex(key);
      printf("plain text:");
      phex(plain_text + i * (uint8_t) 16);
      printf("reference module result:");
      AES_ECB_encrypt(&ctx, plain_text + (i * 16));
      phex(plain_text + (i * 16));
      printf("rocketchip result:");
      phex(plain_text + (i * 16));
      printf("verification success:%d\n",count);
...
#define Nb 4
    ...
    #define Nr 8       // The number of rounds in AES Cipher.
#endif
...
static void KeyExpansion(uint8_t* RoundKey, const uint8_t* Key)
{
  ...
  for (i = Nk; i < Nb * (Nr + 1); ++i)
  {
    ...
    if(i%2 == 0){
      tempa[0]= RoundKey[k_n+3];
    }
    else{
      tempa[0]= RoundKey[k_p+3];
    }
    tempa[1]= RoundKey[k+0];
    tempa[2]= RoundKey[k+1];
    tempa[3]= RoundKey[k+2];
...
//   }
    j = i * 4; 
    RoundKey[j + 0] = tempa[0];
    RoundKey[j + 1] = tempa[1];
    RoundKey[j + 2] = tempa[2];
    RoundKey[j + 3] = tempa[3];
    static int debug = 0;
    // if(debug == 0){
    //   debug = 1;
    //   printf("debug:");
    ...
    //   if(i%4 == 0)
    //     printf("\n");
    //   printf("w[%d]((%x)(%x)(%x)(%x))",i,RoundKey[j+0],RoundKey[j+1],RoundKey[j+2],RoundKey[j+3]);
    //   printf(" ");
    // }
    }
    printf("\n");
...
// 10 function mixes the columns of the state matrix
static void MixColumns(state_t* state)
{
  ...
  { 
    ...
    (*state)[i][0] = Multiply(a, 0x02) ^ Multiply(b, 0x01) ^ Multiply(c, 0x03) ^ Multiply(d, 0x01);
    (*state)[i][1] = Multiply(a, 0x01) ^ Multiply(b, 0x02) ^ Multiply(c, 0x01) ^ Multiply(d, 0x03);
    (*state)[i][2] = Multiply(a, 0x03) ^ Multiply(b, 0x01) ^ Multiply(c, 0x02) ^ Multiply(d, 0x01);
    (*state)[i][3] = Multiply(a, 0x01) ^ Multiply(b, 0x03) ^ Multiply(c, 0x01) ^ Multiply(d, 0x02);
  }
}
